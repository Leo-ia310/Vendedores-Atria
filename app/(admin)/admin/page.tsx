...
import { Users2, ShieldCheck, ShoppingBag, Wallet, MessageCircle, AlertTriangle } from "lucide-react";
...
export default function AdminResumen() {
  ...
  const [d, setD] = useState({
    candidatos: 0, vendedores: 0, ventasPendientes: 0, comisionesPendientes: 0,
    noResueltas: 0, montoPendiente: 0,
  });

  useEffect(() => {
    (async () => {
      const [cand, vend, ventas, comis, chat] = await Promise.all([
        contar("Candidatos"), contar("Vendedores"), contar("Ventas"),
        contar("Comisiones"), contar("PreguntasNoResueltas"),
      ]);
      setD({
        ...
        comisionesPendientes: comis.filter((c) => ["pendiente", "aprobada"].includes(String(c.Estado))).length,
        montoPendiente: comis.filter((c) => ["pendiente", "aprobada"].includes(String(c.Estado)))
          .reduce((s, c) => s + Number(c.Monto || 0), 0),
        noResueltas: chat.filter((q) => String(q.Revisado) !== "true").length,
      });
      setCargando(false);
    })();
  }, []);
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <KpiCard label="Preguntas sin resolver" value={d.noResueltas} icon={MessageCircle} tono="info" />
        <KpiCard label="Monto por pagar" value={formatearUSD(d.montoPendiente)} icon={Wallet} />
      </div>

      <div className="mt-6 flex items-start gap-3 rounded-[12px] border border-[color:var(--color-warning)]/30 bg-[color:var(--color-warning-bg)] p-4">
        <AlertTriangle size={18} className="mt-0.5 shrink-0 text-[color:var(--color-warning)]" />
        <p className="text-[13px] leading-6 text-[color:var(--color-warning)]">
          El backend ahora usa Supabase/PostgreSQL. Mantén las credenciales de servidor fuera del
          frontend y revisa periodicamente indices, RLS y rendimiento desde el panel de Supabase.
        </p>
      </div>
    </>
  );
